...
class SafetyMonitorService : Service() {
    ...
    override fun onDestroy() {
        ...
        try {
            com.example.shesafe.utils.EmotionAnalyzer.cleanup()
        } catch (e: Exception) {
    ...
    private fun triggerSos() {
        ...
        if (requireFace) {
            ...
            var decided = false
            // Capture one frame
            try { CameraCapture.startCapturing(this) } catch (_: Exception) {}
            // Evaluate latest after 1500ms
            gateHandler.postDelayed({
                if (decided) return@postDelayed
                decided = true
                // Proceed regardless; face presence will be attached by uploader
                proceedSos()
            }, 1500)
        } else {
            proceedSos()
